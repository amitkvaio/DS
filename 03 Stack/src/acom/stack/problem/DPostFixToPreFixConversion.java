package acom.stack.problem;

import java.util.Stack;

public class DPostFixToPreFixConversion {
	public static void main(String[] args) {
		String postfix = "AB+CD-*";
		String prefix = posttfixToPrefixConversion(postfix);
		System.out.println("Prefix----> "+prefix);
	}
	
	public static String posttfixToPrefixConversion(String str) {
		if (str == null) {
			return null;
		}

		Stack<String> stack = new Stack<String>();
		for (int i = 0; i <= str.length() - 1; i++) {
			char c = str.charAt(i);
			if (isOperator(c)) {
				String op2 = stack.pop();
				String op1 = stack.pop();
				String tempOut = c + op1 + op2;
				stack.push(tempOut);
			} else {
				stack.push(c + "");
			}
		}
		return stack.pop();
	}
	
	public static boolean isOperator(char ch) {
	    return "+-*/%^".indexOf(ch) >= 0;
	}
}

/*
postfix --> AB+CD-*
Prefix --> *+AB-CD

Algoritham for postfix to prefix conversion
*****************************************
1.	Read the prefix expression from left to right
2.  If the symbol is an operand, push it onto the stack.
3.  If the sysmbol is an operator, then pop two operands from the stack 
	create a string by concatenating the two operands and the operator
	between them
	String = (operator + Operand1 + operand2 ).
	Push the resultant string back to the stack.
4.  Repeate above steps 2 and 3 until the end of the prefix expression is reached.
5.  The final string on the stack is the infix expression. 

TC : O(n)
SC : O(N)
*/