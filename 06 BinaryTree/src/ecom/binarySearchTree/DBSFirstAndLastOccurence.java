	...
	public static int getFirstMostOrLeftMostIndex(List<Integer> list, int target) {
		...
		while (startIdx <= endIdx) {
			...
			if (list.get(mid) == target) {
				...
				endIdx = mid - 1; // But We have to look more on left side if there is any more occurrence of a
									// same elements.
				/// return mid;
	...
	public static int getLastMostOrRightMostIndex(List<Integer> list, int target) {
		...
		while (startIdx <= endIdx) {
			...
			if (list.get(mid) == target) {
				...
				startIdx = mid + 1; // But We have to look more on right side if there is any more occurrence of a
									// same elements.
				// return mid;
			} else if (list.get(mid) < target) {
				startIdx = mid + 1;
