package com.hashing.closedHashing.quadraticProbing;

import java.util.Arrays;

public class EQuadraticProbingSearch {
	...
	public static void main(String[] args) {
		...
		quadraticProbingSearch(70);
		quadraticProbingSearch(92);
		quadraticProbingSearch(1000);
		quadraticProbingSearch(700);
	...
	public static void quadraticProbingSearch(int value) {
		int key = value % arrSize;
		int index = key;
		int i = 1;
		...
		while (arr[index] != value) {
			index = (key + i * i) % arrSize;
			if (index == key) {
				System.out.println("Reached to last, search value::" + value + " not found");
				return;
			}
			i++;
		}
